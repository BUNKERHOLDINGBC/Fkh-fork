...
try
{
    ...
    catch
    {
        ...
        Console.WriteLine();
        Console.WriteLine("Could not fetch function metadata. Configure fk8s.settings.json and try again.");
        return 0;
    }
...
static string ResolveEndpoint(string route, CliSettings settings)
{
    ...
    if (string.IsNullOrWhiteSpace(baseUrl))
    {
        throw new InvalidOperationException(
            "No function endpoint configured. Define baseUrl in fk8s.settings.json next to the executable.");
    }
...
{
    var settingsPath = Path.Combine(AppContext.BaseDirectory, "fk8s.settings.json");
