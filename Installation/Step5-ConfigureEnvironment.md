...
Set this to `true` to create AAD App Registrations as multi-tenant, allowing users from other Entra ID tenants to sign in to containers. Only takes effect when `enable_aad_container_auth` is `true`.
