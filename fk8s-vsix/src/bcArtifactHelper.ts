...
type ArtifactType = 'sandbox' | 'onprem';
type ArtifactSelect = 'all' | 'latest' | 'first' | 'closest' | 'current' | 'nextminor' | 'nextmajor' | 'daily' | 'weekly';

// ---------------------------------------------------------------------------
// CDN / blob-URL mapping
// ---------------------------------------------------------------------------

const cdnMap: { old: string; new: string; blob: string }[] = [
...
function replaceCDN(sourceUrl: string, useBlobUrl = false): string {
...
function resolveToCdnHostname(storageAccount: string): string {
...
function getArtifactVersion(artifactUrl: string): string {
  return artifactUrl.split('/')[4];
}

function getArtifactCountry(artifactUrl: string): string {
...
async function queryArtifactsFromIndex(
  cdnHost: string,
  type: ArtifactType,
  versionPrefix = '',
  country = '',
  ...
  if (!country) {
    ...
    countries = (JSON.parse(json) as string[]).filter(c => c !== 'platform');
    sort = true;
  } else {
    countries = [country];
  }

  let platformVersions: Set<string> | undefined;
  ...
  for (const c of countries) {
    ...
    for (const entry of entries) {
      if (!entry.Version.startsWith(versionPrefix)) { continue; }
      ...
      if (!doNotCheckPlatform && !platformVersions?.has(entry.Version)) { continue; }

      if (after || before) {
        if (!entry.CreationTime) { continue; }
        const creationTime = new Date(entry.CreationTime);
        if (after && creationTime <= after) { continue; }
        if (before && creationTime >= before) { continue; }
      }
...
// ---------------------------------------------------------------------------
// Public API: getBcArtifactUrl
// ---------------------------------------------------------------------------

interface GetBcArtifactUrlOptions {
  type?: ArtifactType;
  country?: string;
  version?: string;
  ...
  doNotCheckPlatform?: boolean;
}

async function getBcArtifactUrl(opts: GetBcArtifactUrlOptions = {}): Promise<string[]> {
  let {
    type = 'sandbox',
    country = '',
    version = '',
  ...
  if (type === 'onprem') {
    ...
    if (select === 'daily' || select === 'weekly') { select = 'latest'; }
  }
  ...
  // --- Main path ---
  if (!storageAccount) { storageAccount = 'bcartifacts'; }
  const cdnHost = resolveToCdnHostname(storageAccount);
  const baseUrl = `https://${cdnHost}/${type.toLowerCase()}/`;

  let versionPrefix: string;
  let closestToVersion: number[] | undefined;

  if (select === 'closest') {
    if (!version) { throw new Error('You must specify a version number when you want to get the closest artifact URL.'); }
    const dots = version.split('.').length - 1;
    if (dots !== 3) { throw new Error('Version number must be in the format 1.2.3.4 when you want to get the closest artifact URL.'); }
  ...
  let selected: string[];
  switch (select) {
    case 'all': selected = artifactList; break;
    case 'latest': selected = artifactList.length > 0 ? [artifactList[artifactList.length - 1]] : []; break;
    case 'first': selected = artifactList.length > 0 ? [artifactList[0]] : []; break;
    case 'closest': selected = selectClosest(artifactList, closestToVersion!); break;
...
function selectClosest(artifacts: string[], target: number[]): string[] {
...
export async function determineArtifactUrl(projectSettings: Record<string, unknown>): Promise<string> {
  ...
  if (artifact.startsWith('https://')) {
    ...
    const select = selectStr.toLowerCase() as ArtifactSelect;
    const type = artifactType.toLowerCase() as ArtifactType;
  ...
  if (additionalCountries.length > 0 || country.toLowerCase() !== projectCountry.toLowerCase()) {
    ...
    const atUrl = (await getBcArtifactUrl({
      storageAccount,
      type: artifactType.toLowerCase() as ArtifactType,
      country: 'at',
      version: `${artifactVersion[0]}.${artifactVersion[1]}`,
      select: 'latest',
    }))[0] ?? '';

    if (!atUrl) { throw new Error('Latest AT artifact could not be determined.'); }
...
function getStringList(obj: Record<string, unknown>, key: string): string[] {
  const val = getPropertyIgnoreCase(obj, key);
  if (!val) { return []; }
  if (Array.isArray(val)) { return val.filter(v => typeof v === 'string' && v) as string[]; }
